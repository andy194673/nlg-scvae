...
#		keepValues=False
#		print('dact:', dact) # e.g. inform(name='trattoria contadina';pricerange=moderate), andy
		acttype = dact.split('(')[0]
		slt2vals= dact.split('(')[1].replace(')','').split(';')
		jsact = {'acttype':acttype,'s2v':[]}
		for slt2val in slt2vals:
			if slt2val=='': # no slot, e.g. goodbye(), andy
...
#		print('jsact:', jsact)
		'''
		jsact 
		if keepValues: True => {'acttype': 'inform', 's2v': [('name', 'trattoria contadina'), ('pricerange', 'moderate')]}, andy
		if keepValues: False=> {'acttype': 'inform', 's2v': [('name', '_'), ('pricerange', '_')]}
		'''
#		sys.exit(1)
		return jsact
...
class SoftDActFormatter(DActFormatter):
	...
	def format(self,dact):
		'''
		input
			dact: inform(name='trattoria contadina';pricerange=moderate;name='a b c';food)
			jsact: {'acttype': 'inform', 's2v': [('name', '_'), ('pricerange', '_'), ('name', '_'), ('food', '?')]}
		output
			feature: [('a', 'inform'), ('food', '?'), ('name', '_1'), ('name', '_2'), ('pricerange', '_1')]
		'''
#		print('dact:', dact)
		jsact = super(SoftDActFormatter,self).parse(dact) # with keepValues=False
#		print('jsact:', jsact)
		mem = {}
		feature = []
		for sv in jsact['s2v']:
			s,v = sv
			if s==None: # no slot no value, e.g. goodbye
				continue
