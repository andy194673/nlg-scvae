...
from models.layers.decoder_deep import DecoderDeep
from models.masked_cross_entropy import *
USE_CUDA = True

class LM_deep(nn.Module):
	def __init__(self, dec_type, input_size, output_size, hidden_size, d_size, n_layer=1, dropout=0.5, lr=0.001, overgen=1):
	...
	def forward(self, input_seq, dataset, conds_batches, gen=False):
		...
		if self.dec_type == 'sclstm':
			...
			self.output_all, decoded_words = self.dec(input_seq, dataset, last_hidden_t0=last_hidden, last_dt_t0=conds_batches, gen=gen, random_sample=self.random_sample)
		else:
